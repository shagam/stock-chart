...
const BasicTable = (props) => {
  ...
  const servList = [process.env.REACT_APP_AWS_IP, process.env.REACT_APP_LOCAL_SERV_IP, process.env.REACT_APP_AWS_IP_, '10.100.102.4',];
  //   'localhost', process.env.REACT_APP_SERV_EXT];
...
// "xyValue", "chart", 'chart1', 'alpha','api', "aux","date","spikes","month","target"];
  ...
  "target","info_date","gain_date","deep","recoverWeek","deepDate","priceDivHigh","verify_1"] // ,"target"
        ...
        {chartSymbol && <div>
          ...
          {<div>
            <div style={{display:'flex'}}>
              ...
              <input style={{marginLeft: '0px'}}  type="radio" name="mon" value='none' id='7' checked={analyzeTool==='none'} onChange={onOptionChange}/> <div style={{color:'red'}}>none</div> &nbsp;
            </div>
