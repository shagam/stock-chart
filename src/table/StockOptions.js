...
function StockOptions (props) {
  ...
  function historecalOptionQuote (row) { 
    ...
    .then ((result) => {
      ...
      if (log)
        ...
        optionHistoryFiltered['strikeDiff'] = []
        ...
        for (let i = 0; i < optionHistoryFiltered.expiration.length; i++) {
          ...
          if (i > 0)
            optionHistoryFiltered['strikeDiff'][i] = (optionHistoryFiltered.mid[i] - optionHistoryFiltered.mid[i - 1]).toFixed(3)
        ...
        if (! columnShow.includes('updated'))
           setColumnShow ([...columnShow, 'updated', 'stockPrice','mid/price','strikeDiff','exprDiff'])
     })
    .catch ((err) => {
  ...
  function titleGet (header) {
    ...
    if (header === "iv") return 'Implied volatility, expected fluctuation of underlying share price'
    if (header === "delta") return 'Delta sensitivity, change in option price for $1 change in share price'
    if (header === "gamma") return 'Gamma sensitivity, change in delta for $1 change in share price'
...
// (26) ['s', 'optionSymbol', 'underlying', 'expiration', 'side', 'strike', 'firstTraded', 'dte', 'updated', 'bid', 'bidSize', 'mid', 'ask', 'askSize', 'last', 'openInterest', 'volume', 'inTheMoney', 'intrinsicValue', 'extrinsicValue', 'underlyingPrice', 'iv', 'delta', 'gamma', 'theta', 'vega']
  ...
  function optionPremium (expirationsArray, strikeArray) {
    ...
    .then ((result) => {
      ...
      if (!columnShow.includes('strikeDiff')) // if strikeDiff is not in columnShow, add it
 ...
 function strikeRowClick(rowId)  { 
  ...
  function expitrationDiffCalc (premiumArray, OptionQuoteFiltered, rowStart) {
    if (rowStart === 0) {
      ...
      return undefined; // first row, no previous row to compare
    }
    ...
    if (expirationPrevLastIndex === undefined) {
      OptionQuoteFiltered['exprDiff'][rowStart] = undefined;
      return;
    }
    ...
    if (expirationPrevFirstIndex === undefined) {
      OptionQuoteFiltered['exprDiff'][rowStart] = undefined;
      return;
    } 
  ...
  function  getOptionsInfoFromServer () {
    ...
    .then ((result) => {
      ...
      for (let i = 0; i < rows; i++) {
        ...
        const expirationDateValue = props.stockPrice * ((estimatedYearlyGain) / 100 + 1) ** (dte / 365) / (props.priceDivHigh ** config.priceDivHighFactor); 
  ...
  function cellColor (line, attrib) {
    ...
    else if (attrib === 'ask' || attrib === 'strikeDiff' || attrib === 'exprDiff' || attrib === 'bid' || attrib === 'mid' || attrib==='last') {
      if (line > 0 && optionQuote.expiration[line] === optionQuote.expiration[line - 1] ) {
    ...
    if (line ===0 || optionQuote.expiration[line] !== optionQuote.expiration[line - 1])
      return {backgroundColor: 'lightlightpink', color: 'black', fontWeight: 'bold'};
        ...
        {strikeSelected === -1 && strikeArray.length > 0 && <div style={{color: 'magenta'}}> Please select an strike price </div>}
                ...
                <tbody>
                  {strikeArray.map((strike, index) =>{
                    ...
                    <tr key={index}
                      ...
                      <td style={{...ROW_SPACING, width: '100px'}}>{(strike/props.stockPrice).toFixed(2)}  </td> 
                    </tr>
                    )
                  })}
                </tbody>
          ...
          {optionQuote && optionQuote.expiration && <div>
              <div style={{display: 'flex', fontWeight: 'bold'}}>
                <div style={{color: 'magenta'}}>{props.symbol} </div> &nbsp; &nbsp; 
                stockPrice={props.stockPrice} &nbsp; &nbsp; price/High={props.priceDivHigh}  &nbsp; &nbsp; {! isNaN(belowBubble) &&<div>price/bubblePrice={belowBubble.toFixed(3)}</div>} &nbsp; &nbsp;
              </div>
              <div style={{display: 'flex', fontWeight: 'bold'}}>
